...
After(async function () {
  ...
  try {
    if (context.alunosArmazenados && context.alunosArmazenados.length > 0) {
      console.log(`  Limpando ${context.alunosArmazenados.length} alunos do arquivo...`);
      // Aqui seria onde limparia o arquivo, mas como a API não existe, pulamos
    }
  } catch (erro) {
...
Given('que um aluno com CPF {string} já existe', async function (cpf: string) {
  ...
  // Armazenar um aluno fictício em memória (simulando persistência)
...
Then('o aluno deve conter todos os campos preenchidos', async function () {
  ...
  for (const campo of camposEsperados) {
    expect(context.ultimaResposta).to.have.property(
      campo,
      `Campo '${campo}' não encontrado na resposta`
    );
  }
